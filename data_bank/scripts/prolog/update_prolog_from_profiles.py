...
def plant_atoms_from_facts(missing: dict[str, list[str]]) -> list[str]:
    atoms: set[str] = set()
    first_arg_pattern = re.compile(r"^[a-z_][a-z0-9_]*\(([^,\)\s]+)")
    ...
    for key, facts in missing.items():
        ...
        for fact in facts:
            if key == "alias_fact":
                match = alias_pattern.match(fact)
            else:
                match = first_arg_pattern.match(fact)
...
def new_plant_atoms_from_preview(missing: dict[str, list[str]]) -> list[str]:
    atoms: set[str] = set()
    pattern = re.compile(r"^plant\(([^,\)\s]+)\)\.")

    for fact in missing.get("plant_fact", []):
...
def write_preview(project_root: Path, missing: dict[str, list[str]], preview_path: Path) -> None:
    affected_plants = plant_atoms_from_facts(missing)
    new_plants = new_plant_atoms_from_preview(missing)
    ...
    lines = [
        ...
        "## Summary",
        "",
        f"Affected plants with pending facts: {', '.join(affected_plants) if affected_plants else '(none)'}",
        f"New plants in preview: {', '.join(new_plants) if new_plants else '(none)'}",
        "",
    ]
    ...
    print(f"[PREVIEW] affected plants: {', '.join(affected_plants) if affected_plants else '(none)'}")
    print(f"[PREVIEW] new plants: {', '.join(new_plants) if new_plants else '(none)'}")
...
def main() -> None:
    ...
    if not profiles_dir.is_absolute():
        ...
        # not project_root, because data_bank may live inside the project.
