...
Runs the full PNW pest data pipeline:

1. Build PNW pest seed
...
For testing only first 5 pests_ver01:
...
def main() -> None:
    ...
    parser.add_argument("--update-prolog", action="store_true")
    parser.add_argument("--apply-prolog", action="store_true")
    ...
    run(
        [
            py,
            "scripts/pest/pnw_ver01_plant_based/build_pnw_pest_seed.py",
            "--sleep",
            str(args.sleep),
        ],
    ...
    extract_cmd = [
        py,
        "scripts/pest/pnw_ver01_plant_based/extract_pnw_pest_profiles.py",
        "--sleep",
        str(args.sleep),
    ]
