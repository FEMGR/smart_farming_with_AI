#!/usr/bin/env python3
"""
enrich_plants_with_disease_ids.py

Purpose:
- Read normalized disease_bank.json.
- Match diseases to normalized plant profiles.
- Add only lightweight disease relationships to each plant profile:

    pests_and_diseases.known_disease_ids

- Remove old/heavy field:

    pests_and_diseases.known_diseases

- Remove resolved missing field:

    pests_and_diseases.known_diseases
    pests_and_diseases.known_disease_ids

Design decision:
- Plant profile stores only disease IDs.
- disease_bank.json stores disease details.
- Prolog later uses disease IDs to look up symptoms/treatments.
...
def save_json(path: Path, data: Any) -> None:
    """
    Save JSON without sort_keys=True.

    This preserves section order from reorder_profile_sections().
    """
...
def reorder_pests_and_diseases(pests: Dict[str, Any]) -> Dict[str, Any]:
    """
    Keep pests_and_diseases readable and lightweight.
...
def reorder_profile_sections(profile: Dict[str, Any]) -> Dict[str, Any]:
    """
    Keep plant profile sections in a stable readable order.
    """
...
def collect_strings_recursive(value: Any) -> Set[str]:
    """
    Collect normalized string tokens from selected profile structures.

    Used for flexible plant group matching.
    """
    ...
    if isinstance(value, dict):
        for key, nested in value.items():
            ...
            # Avoid collecting long prose/source values.
            if key_norm in {
                "description",
                "notes",
                "source_url",
...
def plant_identity_tokens(profile: Dict[str, Any], file_path: Path) -> Set[str]:
    """
    Build identity tokens for matching disease plants_affected.

    Supports:
    - top-level plant_atom, name, common_name, scientific_name, genus
    - identity.common_name, identity.scientific_name, identity.genus
...
def plant_group_tokens(profile: Dict[str, Any]) -> Set[str]:
    """
    Extract group/category tokens from plant profile.

    This supports disease_bank.plant_groups_affected matching.
    """
...
def normalize_disease_bank(raw_bank: Any) -> Dict[str, Dict[str, Any]]:
    """
    Supports both:
    - disease_bank as object: { disease_id: {...} }
    - disease_bank as list: [{ disease_id: ... }]
    """

    bank: Dict[str, Dict[str, Any]] = {}

    if isinstance(raw_bank, dict):
        items = raw_bank.items()
        ...
        for key, value in items:
            ...
            normalized["disease_id"] = disease_id
            bank[disease_id] = normalized
    ...
    if isinstance(raw_bank, list):
        for item in raw_bank:
            ...
            disease_id = to_snake(item.get("disease_id") or item.get("display_name"))
    ...
    raise RuntimeError("Disease bank must be a JSON object or list.")


def disease_matches_plant(
    disease: Dict[str, Any],
    identity_tokens: Set[str],
    group_tokens: Set[str],
) -> bool:
    plants_affected = set(clean_atom_list(disease.get("plants_affected")))
    groups_affected = set(clean_atom_list(disease.get("plant_groups_affected")))
...
def find_matching_disease_ids(
    ...
    for disease_id, disease in disease_bank.items():
        if disease_matches_plant(disease, identity_tokens, group_tokens):
...
def remove_resolved_missing_fields(profile: Dict[str, Any]) -> Dict[str, Any]:
    """
    Remove old disease missing fields after disease enrichment.

    Since we keep plant profiles lightweight, known_diseases should no longer
    be required.
    """
    ...
    updated_missing = [field for field in missing_fields if field not in fields_to_remove]

    profile["missing_fields"] = updated_missing
...
def enrich_profile_with_diseases(
    ...
    """
    Add disease IDs to one plant profile.

    remove_unmatched_existing=False:
        Keep existing known_disease_ids and add matched IDs.

    remove_unmatched_existing=True:
        Replace known_disease_ids with only IDs currently matched from disease_bank.
    """

    matched_ids = find_matching_disease_ids(profile, profile_path, disease_bank)
    ...
    existing_ids = clean_atom_list(pests.get("known_disease_ids"))
    ...
    pests["known_disease_ids"] = new_ids

    # Keep lightweight. Remove full disease details.
    pests.pop("known_diseases", None)
...
def enrich_all_profiles(
    ...
    disease_bank = normalize_disease_bank(raw_bank)
    ...
    for profile_path in profile_paths:
        ...
        updated, changed, matched_ids = enrich_profile_with_diseases(
            profile=profile,
        ...
        details.append(
            {
                ...
                "matched_disease_ids": matched_ids,
                "changed": changed,
            }
        )
    ...
    return {
        ...
        "disease_bank_entries": len(disease_bank),
        "details": details,
    }
...
def print_summary(summary: Dict[str, Any], dry_run: bool) -> None:
    print("")
    print("========== Plant Disease ID Enrichment Summary ==========")
    print(f"Dry run              : {dry_run}")
    print(f"Disease bank entries : {summary['disease_bank_entries']}")
    ...
    for item in summary["details"]:
        if not item["changed"] and not item["matched_disease_ids"]:
            continue

        plant_file = Path(item["plant_file"]).name
        disease_ids = ", ".join(item["matched_disease_ids"]) or "-"
        changed = item["changed"]

        print(f" - {plant_file}: {disease_ids} | changed={changed}")
        shown += 1

        if shown >= 60:
            break

    remaining = len([item for item in summary["details"] if item["changed"] or item["matched_disease_ids"]]) - shown
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Add lightweight known_disease_ids to plant profiles from central disease bank.")
    ...
    parser.add_argument(
        "--replace",
        action="store_true",
        help=("Replace existing known_disease_ids with current disease bank matches. " "Default is append/merge."),
    )
    ...
    if args.dry_run:
        ...
        print("[OK] Plant profiles updated with lightweight known_disease_ids.")
