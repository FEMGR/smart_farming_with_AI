"""Train, evaluate, compare, and persist Computer Vision disease detection models."""

# ai/tasks/disease_detection/train.py
...
from ai.core.vision.training import train_vision_model, VisionTrainingConfig
from ai.models.vision import build_resnet, build_vit, build_custom_cnn
...
# Task Configurations
DEFAULT_DATA_DIR = PROJECT_ROOT / "datasets" / "raw" / "disease_images"
ARTIFACT_DIR = PROJECT_ROOT / "artifacts" / "disease_detection"
TASK_NAME = "disease_detection"
TASK_LABEL = "Plant Disease Vision Detection"
...
def prepare_vision_data(data_dir: Path = DEFAULT_DATA_DIR, batch_size: int = 32, img_size: int = 224) -> dict[str, Any]:
    """Load image dataset directory structure into PyTorch DataLoaders."""
...
def interactive_loop(dry_run: bool = False) -> None:
    menu = MenuRunner(
        title="Plant Disease Vision Detection Training Menu",
        items=[
            MenuItem(
                key="1",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Manager for Disease Detection Vision Models")
