"""Train, evaluate, compare, and persist Computer Vision plant disease detection models."""
...
ARTIFACT_DIR = PROJECT_ROOT / "ai" / "artifacts" / "disease_detection"
TASK_NAME = "disease_detection"
TASK_LABEL = "Plant Disease Detection Vision Pipeline"
...
def prepare_vision_data(data_dir: Path = DEFAULT_DATA_DIR, batch_size: int = 32, img_size: int = 224) -> dict[str, Any]:
    """Load plant disease image dataset directory structure into PyTorch DataLoaders."""
...
def interactive_loop(dry_run: bool = False) -> None:
    menu = MenuRunner(
        title="Disease Detection Vision Training Menu",
        items=[
            MenuItem(
                key="1",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Manager for Plant Disease Detection Vision Models")
