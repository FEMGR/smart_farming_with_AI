...
Default pipeline:
    scripts/extract_fpi.py
    scripts/extract_pfaf.py
    scripts/enrich_gbif.py
    scripts/merge_profiles.py
    scripts/build_indexes.py
    scripts/validate_data_bank.py

Optional:
    scripts/enrich_perenual.py

This script does NOT call export_to_prolog.py by default.
You can enable it with --export-prolog.
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--include-perenual",
        action="store_true",
        help="Also run scripts/enrich_perenual.py. Usually unnecessary because snapshots already came from Perenual.",
    )

    parser.add_argument(
        "--export-prolog",
        action="store_true",
        help="Run scripts/export_to_prolog.py after validation.",
    )
    ...
    pipeline = [
        [py, "scripts/extract_fpi.py", "--plants", plants_arg],
        [py, "scripts/extract_pfaf.py", "--plants", plants_arg],
        [py, "scripts/enrich_gbif.py", "--plants", plants_arg],
    ]

    if args.include_perenual:
        pipeline.append([py, "scripts/enrich_perenual.py", "--plants", plants_arg])

    pipeline.extend(
        [
            [py, "scripts/merge_profiles.py", "--plants", plants_arg],
    ...
    if args.export_prolog:
        pipeline.append([py, "scripts/export_to_prolog.py"])
