...
Architecture Role:
- Acts as a dedicated interface for the frontend_streamlit to manage plant data on the backend.
...
Frontend UI action (e.g., "Add Plant", "View Plants", "Get Recommendations")
        ↓
Corresponding function in `frontend_streamlit/api/plants.py` is called
        ↓
...
Backend processes the request and returns data or status
        ↓
Data (e.g., list of plants, a single plant object, recommendations) is returned to the frontend_streamlit UI
"""

# frontend_streamlit/api/plants.py
