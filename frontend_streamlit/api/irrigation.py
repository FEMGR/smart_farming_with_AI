...
allowing the frontend_streamlit to retrieve plants that need watering and to mark plants as watered.
...
Architecture Role:
- Acts as a dedicated interface for the frontend_streamlit to manage irrigation data on the backend.
...
Backend processes the request and returns a list of plants needing water
        ↓
List of plants is returned to the frontend_streamlit UI for display
...
Backend updates the watering status
        ↓
Confirmation message or updated data is returned to the frontend_streamlit UI
"""

# frontend_streamlit/api/irrigation.py
