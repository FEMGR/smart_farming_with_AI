...
Architecture Role:
- Acts as a dedicated interface for the frontend_streamlit to manage location data on the backend.
...
Frontend UI action (e.g., "Add Location", "View Locations")
        ↓
Corresponding function in `frontend_streamlit/api/locations.py` is called
        ↓
...
Backend processes the request and returns data or status
        ↓
Data (e.g., list of locations, a single location object) is returned to the frontend_streamlit UI
"""

# frontend_streamlit/api/locations.py
